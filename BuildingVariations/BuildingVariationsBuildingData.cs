...
namespace BuildingVariations
{
    ...
    public static class BVBuildingData
    {
        ...
        public static bool IsSubmeshEnabled(ushort building, BuildingInfo.MeshInfo submesh){
            ...
            // check if no variations contain submesh. if no variations contain submesh, return true (enabled). this option is for qualithy of life
            bool doesNotContain = true;
            for (int i = 0; i < PotentialVariationsMap[key].Count; i++){
                if(PotentialVariationsMap[key][i].m_enabledSubMeshes.Contains(submesh.m_subInfo.name)){
    ...
    public class BVBuildingMonitor : BuildingExtensionBase
    {
        public override void OnBuildingReleased(ushort id)
        {
            // Bulldozed/deleted buildings must be reset to the default variation.
            BVBuildingData.IngameBuildingVariationMap[id] = 0;
        }
    }
}
